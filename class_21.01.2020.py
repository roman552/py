...
import random
"""Класс боец. Имеет атрибут здоровье."""
class warrior():
    health=100
    """Метод бой. Принимает первого и второго бойца."""
    def fight(self,first,second):
        self.first=first
        self.second=second
        """Цикл пока здоровье первого и второго бойца равно больше нуля."""
        while first.health>0 and second.health>0:
            """Рандом выбирает какой боец будет бить."""
            fighter=random.randint(1,2)
            
            """Если рандом выбрал первого бойца, наносится урон второму бойцу."""
            if fighter==1:
                
                second.health-=20
                print('Олег наносит урон Игорю')
                """Вывод здоровья обоих бойцов"""
                print('Олег','=',first.health,'HP')
                print('Игорь','=',second.health,'HP')
                """Если здоровье второго бойца равно 0 выводится победитель."""
                if second.health==0:
                    print('Олег выйграл!')
           
            
                """Если рандом выбрал второго бойца, наносится урон первому бойцу."""
            else:
                first.health-=20
...
"""Класс содержащий общие для героев и солдат атрибуты"""
class Units():
	def __init__(self,team,id):
		self.team=team
		self.id=id
"""Дочерний класс герой"""
class Hero(Units):
